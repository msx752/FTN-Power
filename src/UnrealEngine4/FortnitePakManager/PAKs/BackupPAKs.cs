...
namespace FModel.Methods.PAKs
{
    ...
    {
        private static readonly string BACKUP_FILE_PATH = FProp.FOutput_Path + "\\Backups\\FortniteGame_" + DateTime.Now.ToString("MMddyyyy") + ".fbkp";
              ...
              //  new UpdateMyProcessEvents($"Writing {Path.GetFileName(BACKUP_FILE_PATH)}", "Waiting").Update();
                ...
                using (BinaryWriter writer = new BinaryWriter(fileStream))
                {
                    ...
                    foreach (PAKInfosEntry Pak in PAKEntries.PAKEntriesList)
                    {
                        ...
                        {
                            AESKey = AESUtility.StringToByteArray(FProp.FPak_MainAES);
                        }
